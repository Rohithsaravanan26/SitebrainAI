...
const INSIGHT_STUBS: InsightStub[] = [
  {
    ...
    insight: 'MEP package is running 9 days behind baseline. At current burn rate, final delivery may slip 3–4 weeks.',
    confidence: 84,
  ...
  },
  {
    ...
    insight: 'Near-miss incident frequency increased by 40% on Level 12–14 slab pours over the last 14 days.',
    confidence: 91,
  ...
  },
  {
    ...
    insight: 'Steel fixing crew productivity exceeds planned rate by 6%. Consider re-sequencing to accelerate structure.',
    confidence: 78,
  ...
  },
  {
    ...
    insight: 'N20 rebar stock will reach reorder threshold in approximately 11 working days at current consumption.',
    confidence: 95,
    ...
    >
      <div className="space-y-2">
        {INSIGHT_STUBS.map((insight) => (
          ...
          >
            <div className="flex items-center gap-2 mb-1">
              ...
              <span className={`text-[10px] font-bold uppercase tracking-wider font-mono ${severityText[insight.severity]}`}>
                {insight.category}
              </span>
            ...
            <p className="text-xs text-slate-700 dark:text-slate-300 leading-relaxed">{insight.insight}</p>
          </div>
        ))}
        ...
        <div className="p-2.5 bg-slate-50 dark:bg-slate-800/40 border border-dashed border-slate-300 dark:border-slate-700 rounded-sm text-[11px] text-slate-500 font-mono text-center">
          Connect PyTorch / ONNX computer vision pipeline to generate live field insights from drone footage and progress photos.
        </div>
      </div>
    </WidgetShell>
  );
